...
//		String outputFolder = "data/output/scen1";
		...
		double netSurvi[][][] = new double[u.length][file + 1][rep];
		double lifetime[][][] = new double[u.length][file + 1][rep];
		double movingCost[][][] = new double[u.length][file + 1][rep];
		double avgLifetime[][][] = new double[u.length][file + 1][rep];

		PSolver.reproductioner = new MFEA_Reproductioner();
		for (int scale = 0; scale < u.length; scale++) {
			Configs.U = u[scale];
...
//					solution.log();
					...
					System.out.println("Done: file" + "_" + k + ", u = " + u[scale] + ", seed = " + seed);
				}
			}
		}
